...
from .serializers import (
    PrizesSerializer,
    RedeemedPrizesSerializer,
    RedeemedPrizesQrCodeSerializer,
...
class PrizesViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not request.user.is_partner:
            return Response(
                {"detail": "Only partners can create coupons."},
    ...
    def perform_create(self, serializer):
        ...
        serializer.save(
            ...
            generated_by_slug=self.request.user.company_name_slug,
        )
...
class RedeemedPrizesViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if self.request.user.is_partner:
            return Response(
                {"detail": "Only users can redeem coupons."},
                status=status.HTTP_403_FORBIDDEN,
            )

        prize_id = request.data.get(
            "prize"
        )  # Note que troquei 'request.get' por 'request.data.get'

        try:
            prize = Prizes.objects.get(id=prize_id)
        except Prizes.DoesNotExist:
            return Response(
                {"detail": "Prize not found."}, status=status.HTTP_404_NOT_FOUND
            )

        if prize.times_to_be_used == 0:
            return Response(
                {"detail: No coupouns left, choose another"},
        ...
        try:
            ...
            history = History(
                ...
                - prize.cost_in_points,  # Você precisa definir 'some_total_points'
    ...
    def get_queryset(self):
        if self.request.user.is_authenticated:
            ...
            if prize_filter is None or prize_filter == "":
                return (
                    RedeemedPrizes.objects.none()
                )  # Return an empty queryset if prize filter is not provided or empty
